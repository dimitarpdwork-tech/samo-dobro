...
def build_frame(cfg: dict, article: dict) -> Image.Image:
    ...
    # Fit the photo to the frame WIDTH and band it, rather than filling the
    # whole vertical frame.
    #
    # Cover-fitting a 1200x672 article image into 1080x1920 means stretching
    # 672px of height to 1920 — a 2.86x upscale that turns a perfectly good
    # photo into visible mush. Article images are 16:9 by design, and no
    # amount of resampling invents the missing pixels.
    #
    # Scaling to width instead is a 0.9x resize: the photo shows at
    # essentially native resolution and stays sharp. The bands above and below
    # are filled by stretching the photo's own edge rows, so the seam is soft
    # and the headline gets a clean area to sit on.
    scale = W / photo.width
    ph = round(photo.height * scale)
    photo = photo.resize((W, ph), Image.LANCZOS)
    band_top = int(H * 0.28)
    top_strip = photo.crop((0, 0, W, 1)).resize((W, band_top), Image.NEAREST)
    canvas.paste(top_strip, (0, 0))
    canvas.paste(photo, (0, band_top))
    bottom_h = H - (band_top + ph)
    if bottom_h > 0:
        bot_strip = photo.crop((0, ph - 1, W, ph)).resize((W, bottom_h), Image.NEAREST)
        canvas.paste(bot_strip, (0, band_top + ph))
