...
def build_writing_prompt(cfg: dict, story: dict, full_text: str | None, use_search: bool = False) -> str:
    ...
    if use_search:
        ...
        context_rule = (
            ...
            "- Write this as a genuine CAUSAL explanation, not vague praise ('this is inspiring', 'this shows "
            "progress'). Explain the actual mechanism or reason: why does this matter, what changes because "
            "of it, what would be different without it. For example, for a returning species: what specific "
            "ecological role it plays and what breaks down in its absence, in general terms. For new "
            "equipment or infrastructure: what capability it adds that didn't exist before, and for whom.\n"
            "- Make the passage self-contained — someone should be able to read ONLY this part and come away "
            "with a complete, accurate answer to 'why does this matter', without needing the rest of the "
            "article. Aim for roughly 100-160 words in this section — long enough to be substantive, short "
            "enough to stand alone as a citable unit.\n"
            "- Use only general, well-established knowledge you are confident is accurate — the kind of "
            "explanation a knowledgeable person could give without looking anything up. Do NOT invent specific "
            "new numbers, dates, names, or program details beyond what's in the source above; if you're not "
            "sure of a specific fact, explain the general principle or mechanism instead rather than guessing."
        )
